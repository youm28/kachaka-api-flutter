...
ROUTE_PATTERNS = {
    ...
    ("充電ドック", "1"): {
        "route_left":   ["a"],              # 左側を直進
        "route_center": ["b"],              # 中央通路を経由して左へ
        "route_right":  ["e", "c", "d"]     # 右下→右上→上部経由で左へ (大回り)
    },
    # 充電ドック → 目的地2 (右上)
    ("充電ドック", "2"): {
        "route_left":   ["a", "d"],         # 左を上がって上から回り込む
        "route_center": ["b", "d"],         # 中央を上がって上から向かう
        "route_right":  ["e", "c"]          # 右下から右通路を北上
    },
    # 充電ドック → 目的地3 (右下)
    ("充電ドック", "3"): {
        "route_left":   ["a", "d", "c"],    # 左上まで行ってから右側を南下 (大回り)
        "route_center": ["b", "c"],         # 中央へ出てから右へ
        "route_right":  ["e"]               # 右下へ直行
    },
    # 充電ドック → 目的地4 (中央上)
    ("充電ドック", "4"): {
        "route_left":   ["a", "d"],         # 左から回り込む
        "route_center": ["b"],              # 中央を直進
        "route_right":  ["e", "c", "d"]     # 右から回り込む
    },
    # 充電ドック → 目的地5 (左下・近場)
    ("充電ドック", "5"): {
        "route_left":   [],                 # 直行 (左エリア内)
        "route_center": ["b"],              # 中央へ一度出る
        "route_right":  ["e"]               # 右下を経由して戻る
    },
    # 充電ドック → 目的地6 (中央下)
    ("充電ドック", "6"): {
        "route_left":   ["a"],              # 左へ出てから向かう
        "route_center": ["b"],              # 中央奥へ行って戻る
        "route_right":  ["e"]               # 右下を経由
    },
    ...
    ("1", "2"): { # 左上 → 右上
        "route_left":   ["d"],              # 上部を通過 (最短に近い)
        "route_center": ["b", "c"],         # 中央へ降りてから右へ
        "route_right":  ["a", "e", "c"]     # 左を南下し、下部を回って右を北上 (最大迂回)
    },
    ("1", "3"): { # 左上 → 右下
        "route_left":   ["a", "e"],         # 左を南下して下から向かう
        "route_center": ["b", "c"],         # 中央を横切る
        "route_right":  ["d", "c"]          # 右上へ出てから南下
    },
    ("1", "4"): { # 左上 → 中央上
        "route_left":   ["a", "b"],         # 左へ少し降りて戻る
        "route_center": ["d"],              # 上から入る
        "route_right":  ["d", "c", "b"]     # 右へ回って戻る
    },
    ("1", "5"): { # 左上 → 左下
        "route_left":   ["a"],              # 左通路を南下
        "route_center": ["b"],              # 中央通路を使って南下
        "route_right":  ["d", "c", "e"]     # 右通路を使って大きく回る
    },
    ("1", "6"): { # 左上 → 中央下
        "route_left":   ["a"],              # 左経由
        "route_center": ["d", "b"],         # 中央経由
        "route_right":  ["d", "c", "e"]     # 右経由
    },

    # --- 目的地2 (右上) からの移動 ---

    ("2", "1"): { # 右上 → 左上
        "route_left":   ["c", "e", "a"],    # 右を南下し下を回って左を北上 (最大迂回)
        "route_center": ["c", "b", "a"],    # 中央を横切る
        "route_right":  ["d"]               # 上部を通過
    },
    ("2", "3"): { # 右上 → 右下
        "route_left":   ["d", "a", "e"],    # 左側へ大きく回る
        "route_center": ["b", "e"],         # 中央寄りを通る
        "route_right":  ["c"]               # 右通路を南下
    },
    ("2", "4"): { # 右上 → 中央上
        "route_left":   ["d", "a", "b"],    # 左へ回って戻る
        "route_center": ["d"],              # 上から入る
        "route_right":  ["c", "b"]          # 右へ降りて戻る
    },
    ("2", "5"): { # 右上 → 左下
        "route_left":   ["d", "a"],         # 左通路へ出て南下
        "route_center": ["b"],              # 中央通路を南下
        "route_right":  ["c", "e"]          # 右通路を南下して下から入る
    },
    ("2", "6"): { # 右上 → 中央下
        "route_left":   ["d", "a"],         # 左経由
        "route_center": ["b"],              # 中央経由
    ...
    ("3", "1"): { # 右下 → 左上
        "route_left":   ["e", "a"],         # 下を通って左を北上
        "route_center": ["c", "b", "d"],    # 中央を北上
        "route_right":  ["c", "d"]          # 右を北上して上から入る
    },
    ("3", "2"): { # 右下 → 右上
        "route_left":   ["e", "a", "d"],    # 左側へ大きく回る
        "route_center": ["b", "d"],         # 中央寄りを通る
        "route_right":  ["c"]               # 右通路を北上
    },
    ("3", "4"): { # 右下 → 中央上
        "route_left":   ["e", "a", "d"],    # 左から回り込む
        "route_center": ["c", "b"],         # 中央を北上
        "route_right":  ["c", "d"]          # 右から回り込む
    },
    ("3", "5"): { # 右下 → 左下
        "route_left":   ["e"],              # 下部を直行
        "route_center": ["c", "b"],         # 中央へ上がって降りる
        "route_right":  ["c", "d", "a"]     # 右を上がって左へ大回り
    },
    ("3", "6"): { # 右下 → 中央下
        "route_left":   ["e", "a"],         # 左経由
        "route_center": ["e"],              # 下から入る
        "route_right":  ["c", "b"]          # 右経由
    },
    ...
    ("4", "1"): {
        "route_left":   ["a"],              # 左へ出る
        "route_center": ["b", "a"],         # 中央を下がって左へ
        "route_right":  ["d", "c", "e"]     # 右へ出てぐるっと回る
    },
    ("4", "2"): {
        "route_left":   ["d", "a", "e", "c"], # 左へ出てぐるっと回る
        "route_center": ["b", "c"],         # 中央を下がって右へ
        "route_right":  ["c"]               # 右へ出る
    },
    ("4", "3"): {
        "route_left":   ["a", "e"],         # 左通路を利用
        "route_center": ["b", "c"],         # 中央通路を利用
        "route_right":  ["c"]               # 右通路を利用
    },
    ("4", "5"): {
        "route_left":   ["a"],              # 左通路を利用
        "route_center": ["b"],              # 中央通路を利用
        "route_right":  ["c", "e"]          # 右通路を利用
    },
    ("4", "6"): {
        "route_left":   ["a"],              # 左経由
        "route_center": ["b"],              # 直進
    ...
    ("5", "1"): {
        "route_left":   ["a"],              # 左通路を北上
        "route_center": ["b"],              # 中央通路を北上
        "route_right":  ["e", "c", "d"]     # 右通路を北上
    },
    ("5", "2"): {
        "route_left":   ["a", "d"],         # 左通路を北上
        "route_center": ["b", "d"],         # 中央通路を北上
        "route_right":  ["e", "c"]          # 右通路を北上
    },
    ("5", "3"): {
        "route_left":   ["a", "d", "c"],    # 左を北上してから右へ
        "route_center": ["c"],              # 中央から右へ
        "route_right":  ["e"]               # 下を通って右へ
    },
    ("5", "4"): {
        "route_left":   ["a", "d"],         # 左経由
        "route_center": ["b"],              # 中央経由
    ...
    ("6", "1"): {
        "route_left":   ["a"],              # 左経由
        "route_center": ["b"],              # 中央経由
        "route_right":  ["e", "c", "d"]     # 右経由
    },
    ("6", "2"): {
        "route_left":   ["a", "d"],         # 左経由
        "route_center": ["b", "4"],         # 中央経由
        "route_right":  ["e", "c"]          # 右経由
    },
    ("6", "3"): {
        "route_left":   ["a", "d", "c"],    # 左経由
        "route_center": ["e", "c"],         # 中央経由(下)
        "route_right":  ["e"]               # 右経由(下)
    },
    ("6", "4"): {
        "route_left":   ["a"],              # 左経由
